...
//     }
//
//     // Validate UPC format (simple regex for UPC format)
//     final upcRegex = RegExp(r'^[0-9]{12}$'); // Example for 12-digit UPC
//     if (!upcRegex.hasMatch(_upcController.text)) {
//       _showSnackbar('Invalid UPC format. Please enter a 12-digit UPC.');
//       return;
//     }
//
...
//
//           // Check for specific error message and show snackbar
//           if (e.response?.data['error'] != null &&
//               e.response?.data['error']['message'] == 'Invalid UPC format.') {
//             _showSnackbar('Invalid UPC format. Please try again.');
//           }
//         }
//       } else {
  ...
  Future<void> _submitScanData() async {
    ...
    try {
      ...
      if (response.statusCode == 201) {
       ...
       _showAlert('Failed to submit data: ${responseBody['message']}');
      }
    } catch (e) {
      if (e is DioError) {
        debugPrint('DioError: ${e.message}');
      ...
      body: Padding(
        ...
        child: ListView(
          children: [
            ...
            SizedBox(height: 16),
            _buildTextField(
                _descriptionController, 'Description', TextInputType.text),
            SizedBox(height: 16),
            _buildTextField(
                _departmentController, 'Department', TextInputType.text),
            SizedBox(height: 16),
